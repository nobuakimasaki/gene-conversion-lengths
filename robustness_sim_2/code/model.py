...
def optim_geom(psi_lst, l_lst, M):
    bounds = [(0.0005, 0.3)]  # Bounds for phi
...
def fit_model_M(psi_lst_filt, l_lst_filt, M):
    ...
    n = len(l_lst_filt)   # same for psi_lst_filt
    ...
    k_null = 1
    aic_null = 2 * nll_null + 2*k_null  # 1 parameter
    bic_null  = 2 * nll_null + k_null * np.log(n)
    ...
    mean_sum = 2 / phi_sum
    k_sum = 1
    aic_sum = 2 * nll_sum + 2*k_sum  # 1 parameter
    bic_sum  = 2 * nll_sum + k_sum * np.log(n)
