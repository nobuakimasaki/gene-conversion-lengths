...
res_mixture_hotspot$BIC <- res_mixture_hotspot$AIC - 2*3 + 3*log(nrow(l_psi_hotspot))

res_mixture_nonhotspot <- res_mixture_nonhotspot[1,]
res_mixture_nonhotspot$type <- "nonhotspot"
res_mixture_nonhotspot$BIC <- res_mixture_nonhotspot$AIC - 2*3 + 3*log(nrow(l_psi_nonhotspot))
...
# obtain difference in BIC between the mixture model and geometric model
long_est_hotspot <- long_est %>% filter(type == "hotspot")
long_est_hotspot %>% filter(model == "mixture" & variable == "BIC") %>% select(est) - long_est_hotspot %>% filter(model == "null" & variable == "BIC") %>% select(est)
...
long_est_nonhotspot %>% filter(model == "mixture" & variable == "BIC") %>% select(est) - long_est_nonhotspot %>% filter(model == "null" & variable == "BIC") %>% select(est)
```
