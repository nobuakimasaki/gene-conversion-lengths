...
sim_tracts_mixture <- function(N_gene_conv, p, phi1, phi2, length_chrom) {
  start <- sample(1:length_chrom, N_gene_conv, replace = TRUE)
  
  # Step 1: Sample component labels
  component <- sample(c(1, 2), size = N_gene_conv, replace = TRUE, prob = c(p, 1 - p))
  
  # Step 2: Generate samples from the selected components
  samples1 <- rgeom(sum(component == 1), prob = phi1) + 1
  samples2 <- rgeom(sum(component == 2), prob = phi2) + 1
  lengths <- c(samples1, samples2)
...
# neg_log_lik_mixture_w1 <- function(par, psi_lst, l_lst, M) {
#   phi1 <- par[1]
#   phi2 <- par[2]
#   w1 <- par[3]
#   
