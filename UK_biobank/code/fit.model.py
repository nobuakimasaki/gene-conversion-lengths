...
def bootstrap_once(args):
    ...
    log_path = f"logs/bootstrap_M{M}_seed{base_seed}.log"
...
if __name__ == "__main__":
    ...
    # Run the whole analysis for each M
    for M in [1500, 2000, 3000]:
        # Wrapper that closes over M
        def get_l_psi_wrapper(i, M=M):
            return get_l_psi(
                MAF_df=MAF_df_lst[i],
        ...
        # Parallel per-chromosome preprocessing
        ...
        # Fit original model once (seed == 1), save with M in filename
        if seed == 1:
            res_mixture, res_null, res_sum = model.fit_model_M(psi_lst_filt, l_lst_filt, M)
            res_mixture.to_csv(f"res_mixture_M{M}.csv", index=False)
            res_null.to_csv(f"res_null_M{M}.csv", index=False)
            res_sum.to_csv(f"res_sum_M{M}.csv", index=False)
            l_psi_df = pd.DataFrame(list(zip(l_lst_filt, psi_lst_filt)), columns=["l", "psi"])
            l_psi_df.to_csv(f"l_psi_M{M}.csv", index=False)

        # Prepare bootstrap args (include M)
        args_list = [(psi_lst_filt, l_lst_filt, M, seed * 10000 + i, seed, i + 1) for i in range(12)]

        # Bootstrap in parallel
        bootstrap_results = []
        with ProcessPoolExecutor() as executor:
            for i, result in enumerate(executor.map(bootstrap_once, args_list)):
                print(f"[M={M}] Finished bootstrap iteration {i + 1}")
                result["bootstrap"] = f"boot_{i + 1}"
                bootstrap_results.append(result)

        bootstrap_df = pd.concat(bootstrap_results, ignore_index=True)
        bootstrap_df.to_csv(f"bootstrap_res_seed{seed}_M{M}.csv", index=False)